      ...
      if(! $connection) {
        ...
        if (mysqli_num_rows($result) > 0) {
          ...
          while($row = mysqli_fetch_assoc($result)) {
            echo '<tr><td>' . $row["firstname"]. '</td> <td>' . $row["lastname"]. '</td> <td>' . $row["age"]. '</td>  <td>' . $row["sex"]. '</td> <td>' . $row["health_card"]. '</td> <td>' . $row["emergency_contact"]. '</td> <td>' . '<a href="patient_profile1.php?id='.$row["id"].'">More Info</a>' . '</td></tr>';
          }
          echo '</table>';
        } else {
          echo "0 records.";
